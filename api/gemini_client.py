...
#     if there are multiple student records, ignore everything mentioned above, rather just provide 1 comprehensive summary,instead of multiple individual company analysis.
#     """
...
def generate_summary(student_data):
    ...
    # Case 2: If no company is specified, generate a consolidated summary
        ...
        overall_rounds_cleared = set()
        overall_rounds_failed = set()
        ...
        for record in student_data:
            for round_name, status in {
                ...
                if status:
                    overall_rounds_cleared.add(round_name)
                elif status is False:
                    overall_rounds_failed.add(round_name)
        ...
        Interview Rounds Cleared Most Often: {', '.join(overall_rounds_cleared) if overall_rounds_cleared else 'None'}
        Interview Rounds Failed Most Often: {', '.join(overall_rounds_failed) if overall_rounds_failed else 'None'}
